...
def replay_branch(
    task: BranchTask, agent: BaseAgent, output_root: Path, rollout_index: int = 0
) -> ReplayResult:
    ...
    initial = snapshot_texts(workspace)
    started = time.perf_counter()
    messages = list(task.prefix_messages)
    trajectory: list[dict[str, Any]] = []
    observation: ToolObservation | dict[str, object] = {
    ...
    if task.mode == "forced":
        ...
        observation = executor.execute(
            forced, remaining_time_seconds=task.time_limit_seconds
        )
        steps += 1
        messages.extend(
            [
                {
                    "role": "assistant",
                    "tool_call": task.forced_action,
                    "metadata": {"forced": True},
                },
                {
                    "role": "tool",
                    "name": forced.action_type.value,
    ...
    while (
        not submitted
        and steps < task.remaining_steps
        and time.perf_counter() - started < (task.time_limit_seconds or 60)
    ):
        state = ToolState(
            steps,
            task.remaining_steps - steps,
            environment.manifest.tool_permissions.enabled_tools(),
            submitted=submitted,
        )
        action = agent.act(observation, state)
        observation = executor.execute(
            action,
            remaining_time_seconds=(task.time_limit_seconds or 60)
            - (time.perf_counter() - started),
        )
        steps += 1
        submitted = action.action_type == ActionType.SUBMIT
        messages.extend(
            [
                {
                    "role": "assistant",
    ...
    result = evaluator.evaluate(
        workspace, environment.manifest, environment.hidden_root
    )
    final_workspace = root / "final_workspace"
    shutil.copytree(workspace, final_workspace)
    write_jsonl(root / "trajectory.jsonl", trajectory)
    write_json(root / "evaluator_result.json", result.to_dict())
    (root / "final_diff.txt").write_text(
        build_unified_diff(initial, snapshot_texts(workspace)), encoding="utf-8"
    )
    outcome = BranchOutcome(
        rollout_id,
        task.task_id,
        ...
        result.success,
        result.subscores,
        result.failure_labels,
        result.diagnostics,
        steps,
        submitted,
        time.perf_counter() - started,
        str(final_workspace),
        str(root / "trajectory.jsonl"),
        {
            **task.metadata,
            "mode": task.mode,
            "forced_action_steps": 1 if task.mode == "forced" else 0,
        },
        rollout_seed=task.metadata.get("rollout_seed"),
        sampling_seed=task.metadata.get("sampling_seed"),
...
def _to_action(value: dict[str, Any] | None) -> Action:
    if value is None:
        raise ValueError("forced branch has no action")
    return Action(ActionType(value["tool"]), dict(value.get("args", {})))


def _observation_text(value: ToolObservation) -> str:
    return "\n".join(
        x for x in (value.message, value.content or "", value.stdout, value.stderr) if x
    )


def _event(
    action: Action, observation: ToolObservation, forced: bool
) -> dict[str, Any]:
    return {
        "action": {"tool": action.action_type.value, "args": action.arguments},
