...
def export_training_data(
    ...
    for comparison in comparisons:
        ranking = comparison.metadata.get(
            "candidate_ranking", list(comparison.candidate_statistics)
        )
        ...
        if (
            format == "sft"
            and best_stats["mean"] >= min_quality
            and best_task.forced_action
        ):
            records.append(
                {
                    "task_id": best_task.task_id,
                    "messages": best_task.prefix_messages,
                    "target_action": best_task.forced_action,
                    "target_return": best_stats["mean"],
                    "source": _source(best_task),
                    "privileged": _privileged(best_task),
                }
            )
            ...
            for rejected_id in ranking[1:]:
                ...
                if (
                    rejected
                    and not (exclude_privileged and _privileged(rejected))
                    and best_task.forced_action
                    and rejected.forced_action
                    and best_stats["mean"] - rejected_stats["mean"] >= min_margin
                ):
                    records.append(
                        {
                            "task_id": best_task.task_id,
                            "state_id": comparison.snapshot_id,
                            "messages": best_task.prefix_messages,
                            "chosen": best_task.forced_action,
                            "rejected": rejected.forced_action,
                            "chosen_return": best_stats["mean"],
                            "rejected_return": rejected_stats["mean"],
                            "return_margin": best_stats["mean"]
                            - rejected_stats["mean"],
                            "chosen_source": _source(best_task),
                            "chosen_privileged": _privileged(best_task),
                            "rejected_source": _source(rejected),
                            "rejected_privileged": _privileged(rejected),
                        }
                    )
        elif format == "critic":
            for candidate_id, stats in comparison.candidate_statistics.items():
                ...
                if (
                    task
                    and task.forced_action
                    and not (exclude_privileged and _privileged(task))
                ):
                    records.append(
                        {
                            "state_id": comparison.snapshot_id,
                            "messages": task.prefix_messages,
                            "action": task.forced_action,
                            "action_source": _source(task),
                            "privileged": _privileged(task),
                            "q_target": stats["mean"],
                            "return_std": stats["std"],
                            "rollout_count": int(stats["count"]),
...
def _source(task: BranchTask) -> str:
    ...
    if source:
        return str(source)
    return (
        "trajectory" if task.metadata.get("candidate_type") == "original" else "unknown"
    )


def export_rl_taskset(
    comparisons: list[BranchComparison],
    tasks: dict[str, BranchTask],
    output_root: Path,
    min_regret: float = 0.2,
) -> list[BranchTask]:
    selected = []
    for comparison in comparisons:
        if comparison.decision_regret < min_regret:
            continue
        source = next(
            (
                task
        ...
        task = BranchTask(
            source.task_id,
            source.branch_group_id,
            source.snapshot_id,
            source.candidate_id,
            "open",
            target.relative_to(output_root).as_posix(),
            source.prefix_messages,
            None,
            source.remaining_steps,
            source.time_limit_seconds,
            source.family,
            source.scenario_id,
            source.difficulty,
            source.seed,
            {**source.metadata, "training_regret": comparison.decision_regret},
        )
        write_json(target / "branch.json", task.to_dict())
        selected.append(task)
    write_jsonl(output_root / "manifest.jsonl", [x.to_dict() for x in selected])
    write_json(
        output_root / "metadata.json",
        {"format_version": "1.0", "mode": "open", "task_count": len(selected)},
    )
    return selected
...
def read_comparisons(path: Path) -> list[BranchComparison]:
    return [
        BranchComparison.from_dict(json.loads(line))
